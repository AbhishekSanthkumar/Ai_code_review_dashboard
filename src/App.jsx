  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Avg Score"       value={metrics.avg_score}       sub="out of 100"      color={scoreColor(metrics.avg_score)} />
        <StatCard label="PRs Reviewed"    value={metrics.total_reviews}   sub="total" />
        <StatCard label="Critical Issues" value={metrics.total_critical}  sub="across all PRs"  color="text-red-400" />
        <StatCard label="Warnings"        value={metrics.total_warnings}  sub="across all PRs"  color="text-amber-400" />
      </div>
        ...
        {trend.length > 0 ? (
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={trend}>
              ...
              />
              <Line type="monotone" dataKey="score" stroke="#34d399" strokeWidth={2} dot={{ fill: "#34d399", r: 4 }} />
            </LineChart>
          </ResponsiveContainer>
        ) : (
      ...
      {/* PR history table */}
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
        <SectionTitle>PR review history — click a row to see comments</SectionTitle>
        {reviews.length > 0 ? (
